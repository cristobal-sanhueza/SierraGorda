...
Periodo = "segundo semestre del 2023"
...
fecha_comienzo_mes_nombre <- spanish_months[as.numeric(fecha_comienzo_mes)]
...
<div class="centered-text">
  <p><b><u>desde `r fecha_comienzo_dia` de `r fecha_comienzo_mes_nombre` al `r fecha_termino_dia` de `r fecha_termino_mes_nombre` `r fecha_termino_year`</u></b></p>
</div>
...
Para evaluar el desempeño del modelo de pronóstico, se dispone de la base de datos para el periodo del `r Periodo`, de las concentraciones de MP10 medidas en la estación Sierra Gorda, la cual se compara con las concentraciones de MP10 pronosticadas por **GEOMET** para igual período.
...
Temperatura_longer %>%
  ...
  scale_x_datetime(date_labels = "%d %b", date_breaks = "8 day")
...
La Figura 3.7 muestra el ciclo horario de las concentraciones de MP10 (µg/m3) medida y modelada en la estación Sierra Gorda para el `r Periodo`. A pesar de que el modelo logra captar el ciclo y tendencia de los niveles de MP10, los valores en la mayoría de las horas está sobreestimada.
...
Para evaluar el desempeño del modelo de pronóstico de MP10, se dispone de la base de datos del periodo del `r Periodo`, de las concentraciones de MP10 medidas en la estación Sierra Gorda, las cuales se comparan con las concentraciones de MP10 pronosticadas por GEOMET para igual período.

La evaluación se realiza por niveles de gestión, y se calcula la Exactitud del pronóstico para cada nivel.

La Tabla III.5 muestra la Exactitud del modelo de pronóstico para el período evaluado en la estación de Sierra Gorda. De ella se observa que en promedio, el acierto del modelo es de un `r label_percent(1)(promedio)`, para el período del `r Periodo`.
