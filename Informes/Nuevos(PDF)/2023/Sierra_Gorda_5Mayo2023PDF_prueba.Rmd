...
MP10_Modelados <- list.files(path = "SIERRA_GORDA/Modelados/pronostico-alerta/sg/2023/5.Mayo/MP10",
                               pattern = "*.xlsx",
                               full.names = TRUE) %>%
  lapply(read_excel)
...
MP10_Modelados <- MP10_Modelados %>% 
  mutate(VALOR = case_when(hora %in% c(2:16) ~ VALOR * 1.5,
                           TRUE ~ VALOR))
...
MP10_Observados <- read_excel("SIERRA_GORDA/Observados/2023/5.Mayo/MP10/Consultas en Linea.xlsx")
...
Met_Modelados <- list.files(path = "SIERRA_GORDA/Modelados/pronostico-alerta/sg/2023/5.Mayo/Meteorologia",    
                            pattern = "*.xlsx",
                            full.names = TRUE) %>% 
  lapply(read_excel) 
...
Met_Observados <- read_excel("SIERRA_GORDA/Observados/2023/5.Mayo/Meteorologia/Consultas en Linea.xlsx")
...
La Figura 3.7 muestra el ciclo horario de las concentraciones de MP10 (µg/m3) medida y modelada en la estación Sierra Gorda para el mes de `r MesDelReporte` del 2023. A pesar de que el modelo logra captar el ciclo y tendencia de los niveles de MP10, los valores en la mayoría de las horas está subestimada. Al igual que el mes pasado, se continuará ajustando el modelo de acuerdo a los cambios de la condiciones de las fuentes.
