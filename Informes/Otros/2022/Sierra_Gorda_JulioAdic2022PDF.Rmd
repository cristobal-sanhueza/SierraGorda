...
MesDelReporte = "julio a diciembre del 2022"
...
Para evaluar el desempeño del modelo de pronóstico, se dispone de la base de datos de `r MesDelReporte`, de las concentraciones de MP10 medidas en la estación Sierra Gorda, la cual se compara con las concentraciones de MP10 pronosticadas por **GEOMET** para igual período.
...
La Figura 3.7 muestra el ciclo horario de las concentraciones de MP10 (µg/m3) medida y modelada en la estación Sierra Gorda para el periodo de `r MesDelReporte`. De ella se observa que el modelo de pronóstico logra reproducir el ciclo diario del MP10 en Sierra Gorda.
...
Para evaluar el desempeño del modelo de pronóstico de MP10, se dispone de la base de datos de `r MesDelReporte`, de las concentraciones de MP10 medidas en la estación Sierra Gorda, las cuales se comparan con las concentraciones de MP10 pronosticadas por GEOMET para igual período.

La evaluación se realiza por niveles de gestión, y se calcula la Exactitud del pronóstico para cada nivel.

La Tabla III.5 muestra la Exactitud del modelo de pronóstico para el período evaluado en la estación de Sierra Gorda. De ella se observa que en promedio, el acierto del modelo es de un `r label_percent(0.1)(promedio)`, para el período de `r MesDelReporte`.
